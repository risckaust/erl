...
class Controller:
	...
	def __init__(self):
		...
		self.x_fence_min_warn = -3 # 5 meters in back of starting position
	...
	def objectPoseCb(self, msg):
		if msg is not None and K.WORKER1SEARCH:
	...
	while not rospy.is_shutdown():
		...
		if K.WORKER1SEARCH:
			...
			if K.worker1_found_flag:
				ropsy.loginfo('Outside worker found!')
				rospy.loginfo('Setting worker position to go to')

				K.positionSp.header.frame_id = 'local_origin'
				K.positionSp.pose.position.x = worker1Sp.pose.position.x
				K.positionSp.pose.position.y = worker1Sp.pose.position.y
