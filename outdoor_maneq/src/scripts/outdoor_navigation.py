...
class Controller:
	...
	def __init__(self):
		...
		self.worker1_search_LEFT_y = K.y_FenceLimit_max - 7.5 # 7.5 meters from the left edge of the field
		self.worker1_search_RIGHT_y = K.y_FenceLimit_min + 7.5 # 7.5 meters from the right edge of the field
		self.worker1_search_NEAR_x = K.building_center_x # DRONE WILL START AT SAME DISTANCE DOWNFIELD AS CENTER OF BUILDING
		self.worker1_search_FAR_x = K.x_FenceLimit_max - 7.5 # STOP 7.5 METERS SHORT OF EDGE OF FIELD
		...
		if K.WORKER1SEARCH:
			...
			if K.worker1_found_flag:
				...
				if K.deliver_attempt_num == 1:
					...
					if (abs(K.current_local_x-K.positionSp.pose.position.x)<.3 and abs(K.current_local_y-K.positionSp.pose.position.y)<.3 and abs(K.current_local_z-K.positionSp.pose.position.z)<.3):
