...
##################fill in those for first indoor test########################################################################################
		...
		self.building_entr1_inside_lat = 0
		self.building_entr1_inside_lon = 0
...
################################################################################################################
	...
	###################################################################################################################
		...
		#self.building_entr1_inside_lat = 0
		#self.building_entr1_inside_lon = 0
		self.building_entr2_inside_lat = 0
		self.building_entr2_inside_lon = 0
		self.building_entr3_inside_lat = 0
		self.building_entr3_inside_lon = 0
	...
	###################################################################################################################
		...
		self.person = roslaunch.parent.ROSLaunchParent(self.uuid, ["/home/risc/catkin_ws/src/yolo_ros_vino/launch/person_yolo_ros_vino.launch"])
		self.tags = roslaunch.parent.ROSLaunchParent(self.uuid, ["/home/risc/catkin_ws/src/yolo_ros_vino/launch/tags_yolo_ros_vino.launch])"])
		self.mapping = roslaunch.parent.ROSLaunchParent(self.uuid, ["/home/risc/catkin_ws/src/my_package/launch/miniMission.launch"])
	...
	def VehicleNavLog(self):
		...
		if (self.previous_timer_sec != self.timer_sec) or (self.current_state != self.previous_cycle_state) or (self.current_signal != self.previous_cycle_signal):
			...
			name = "/home/risc/catkin_ws/src" + str(self.start_time) + ".txt"
			hs = open(name,"a")
			s = "\nTime Stamp (sec):\n"
			s = s + str(self.timer_sec)
	...
	while not rospy.is_shutdown():
		...
		# !!!!!!!!!!!!!!!!!!!!! MAY ALSO WANT TO CHANGE MAX_XY_VEL HERE TOO FROM 1.5 TO 1.0 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			if K.enter_bldg_flag == 0:
				...
				# Remove this part after first indoor test 
				K.unblocked_entrance_x=K.building_entr1_x
				K.unblocked_entrance_y=K.building_entr1_y
			...
			if (not K.worker2_found_flag ):
				...
				K.resetStates()
				K.current_signal = 'FOUND INDOOR PEROSN'
				K.DELIVERAID2=1
				K.person.shutdown()
			...
			K.mapping.start() # ------>>>>>>>>>> WHEN YOU WANT TO START MAPPING TYPE THIS
