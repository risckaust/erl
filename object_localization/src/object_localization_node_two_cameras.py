...
def listener():
    ...
    rospy.Subscriber("/tags/yolo_ros_vino/bounding_boxes", BoundingBoxes, box.cb_bounding_boxes)
...
class bounding_boxes(object):
    def __init__ (self):
        ...
        self.center.pose.position.z =   2     # initlizes z to 2 meter in case mavros fails
    ...
    def cb_bounding_boxes(self,msg):
        if not msg == None:
            for key in self.detections:
                ...
                print "Detected {}".format(self.detections[key]), "occurences of {}".format(key)
	    print '-'*80 
            ...
            for bounding_box in msg.bounding_boxes:
                ...
                if bounding_box.Class == "person":
                    ...
                    self.center.pose.position.z = self.drone_altitude
                else:
                    self.zed_frame_id = "front_color_optical_frame"
                    self.center.pose.position.z = 2 # shortest distance between drone and building in meters
