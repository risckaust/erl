...
class Controller:
	...
	def __init__(self):
		...
		self.waypoint1_x = 15
		self.waypoint1_y = -0.2
		self.waypoint1_z = 2

		self.waypoint2_x = 20.5
		self.waypoint2_y = 11.2
		self.waypoint2_z = 2

		self.waypoint3_x = 36.5
		self.waypoint3_y = 5.6
		self.waypoint3_z = 2
		
		self.x_FenceLimit_max = 64 # 300 meters downfield from the starting position
		self.x_FenceLimit_min = -5 # 5 meters behind the starting the position 
		self.y_FenceLimit_max = 25 # 15 meters to the left of the starting position
		self.y_FenceLimit_min = -25 # 15 meters to the right of the starting position

		self.x_fence_max_warn = 64 # 295 meters downfield from the starting position
		self.x_fence_min_warn = -5 # 5 meters in back of starting position
		self.y_fence_max_warn = 25 # 14 meters to the left of starting position
		self.y_fence_min_warn = -25 # 14 meters to the right of the starting position
		...
		self.building_center_x = 44#10
		self.building_center_y = 8
	...
	def rangessCb(self, msg):
		if msg is not None:
			...
			# rospy.logwarn("Minfront value before cleaned")
			# rospy.logwarn(min(self.front))
			#print(self.front)
	...
	def verifyPOI(self):
		if self.verifyPOI_flag:
			...
			if counter >= 5:
				...
				rospy.logwarn("Outside worker found!")
			elif counter < 5:
				rospy.logwarn("Detected false positive. Continuing search.")
	...
	def check_and_avoid(self,angle):
		...
		if (self.minfront < self.avoid_radius and self.minfront>.5):

			if (self.minleft >= self.minright) and (self.minleft > 10 or self.minleft<0.5) and (self.tried_right ==0):
				if (self.minfront < self.avoid_radius):
			...
			elif (self.minright >= self.minleft) and (self.minright > 5  or self.minright<0.3):
	...
	def check_and_avoid2(self,angle):
		...
		if (self.minfront < self.avoid_radius and self.minfront>.5):
			...
			rospy.logwarn("MinLeft")
			rospy.logwarn(self.minleft)

			rospy.logwarn("MinRight")
			rospy.logwarn(self.minright)

			if (self.minleft > 5 or self.minleft<0.5) and (self.tried_left == 0):
				if (self.minfront < self.avoid_radius ):
					rospy.loginfo("Going Left")
					self.dx=0
					self.dy=20
					# rospy.sleep(2.0)
					self.positionSp.pose.position.x=self.dx*math.cos(angle)-self.dy*math.sin(angle)+self.current_local_x
					self.positionSp.pose.position.y =self.dx*math.sin(angle)+self.dy*math.cos(angle)+self.current_local_y
			...
			elif (self.minright > 5 or self.minright<0.5):
				self.tried_left = 1
				if (self.minfront < self.avoid_radius ):
					rospy.loginfo("Going Right")
					self.dx=0
...
def main():
	...
	while not rospy.is_shutdown():
		...
		if K.WAYPOINT3:
			angle = math.atan2((K.waypoint3_y-K.current_local_y),(K.waypoint3_x-K.current_local_x))
			K.check_and_avoid2(angle)
			if (K.minfront>10 ):
