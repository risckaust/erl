		...
		if msg is not None and self.ENTRANCESEARCH and ((self.ready2capturered + self.ready2captureblue + self.ready2capturegreen)>0):
			...
			# Convert your ROS Image message to OpenCV2
				...
				if self.ready2capturegreen:
					...
					hs.write(g)
					self.ready2capturegreen = 0 #############################################################################################################################Changedto1
...
def main():
	...
	rospy.Subscriber("/laser/scan", LaserScan, K.rangessCb)################################################################################################################################CHANGE BACK TO /scan FOR OUTDOOR TEST
	####################################################################################################
	...
	while not rospy.is_shutdown():
		...
		# !!!!!!!!!!!!!!!!!!!!! MAY ALSO WANT TO CHANGE MAX_XY_VEL HERE TOO FROM 1.5 TO 1.0 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			# reached_building = K.Execute_Waypoint(K.entrance_search_corner1_x, K.entrance_search_corner1_y, K.entrance_search_z, -1000) ############################################################################# Should be uncommented for outdoor test
			reached_building = K.Execute_Waypoint(K.entrance_search_corner1_x, K.entrance_search_corner1_y, 10, -1000)################################################################################################# Simulation only! Remove for outdoor test
			...
			if reached_building:
				...
				#K.tags.start() # ------>>>>>>>>>> WHEN YOU WANT TO START TAG SEARCH####################################################################################################################################Should be uncommented for outdoor test
			...
			if K.enter_bldg_flag == 0:
				...
				if abs((K.unblocked_entrance_x - 2.5) - K.current_local_x) < .5:
					...
					K.WORKER2SEARCH = 1
					############################## START TAG SEARCH NODE ###########################################
					# K.person.start() # ------>>>>>>>>>> WHEN YOU WANT TO START Worker SEARCH############################################################################################################## Should be uncommented for outdoor test!
					# rospy.loginfo("Tag search node started")
					################################################################################################
